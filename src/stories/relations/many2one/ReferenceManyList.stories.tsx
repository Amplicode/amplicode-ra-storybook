        ...
        return (
            <>
                ...
                <GenerationInstructions.InsertOnly>
                    <ReferenceManyField target={subresourceBackReference} reference={subresourceName}>
                        <Datagrid>
                            <TextField source="id" />
    ...
    render: ({ subresourceName, subresourceBackReference, ...props }) => {
        return (
            <>
                ...
                <GenerationInstructions.InsertOnly>
                    <ReferenceManyField target={subresourceBackReference} reference={subresourceName}>
                        <SimpleList leftIcon={(record) => <Typography variant="body1">Id: {record.id}</Typography>} primaryText={record => record.name}/>
                    </ReferenceManyField>
                </GenerationInstructions.InsertOnly>
            </>
        );
